<template>
    <div class="card">
        <div class="card-header d-flex align-items-center justify-content-between flex-wrap row-gap-3">
            ...
            <div class="d-flex col-md-10 justify-content-md-end justify-content-center gap-2">
                <div>
                ...
                <thead class="bg-light-subtle">
                    <tr>
                        <th v-for="(col, index) in columns" :key="index" >
                            <div class="cursor-pointer" :class="col.sort ? 'd-flex align-items-center mb-0 ' + col?.className : ''" @click="handleSort(col?.sort)" v-if="col?.sort">
                                <p :class="col?.className + ' mb-0'">{{ col?.label }}</p>
                                <span v-if="col?.sort">
                                    <i :class="`ms-3 ti ti-chevron-${isAsc ? 'up': 'down'}`" v-if="sort === col?.sort"></i>
        ...
        <Pagination v-if="totalData > 0 && !isLoading" :page="page" :totalPage="totalPage" :startData="startData"
            :endData="endData" :totalData="totalData" :nextPage="nextPage" :prevPage="prevPage" :set-page="setPage">
        </Pagination>
...
const { isLoading, columns, search, endData, limit, nextPage, page, prevPage, setLimit, setPage, setSearch, setSort, sort, startData, totalData, totalPage} = defineProps<{
    isLoading: boolean;
